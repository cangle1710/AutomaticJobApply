...
def reset_db(db_path: Path | str | None = None) -> None:
    """Delete all job records for a fresh pipeline run.

    Called at the start of the discover stage so every execution starts clean.
    The schema is preserved; only data is wiped.
    """
    conn = get_connection(db_path)
    conn.execute("DELETE FROM jobs")
    conn.commit()
    log.info("Database reset — all job records deleted")
