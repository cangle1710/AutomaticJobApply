...
def store_jobspy_results(conn: sqlite3.Connection, df, source_label: str) -> tuple[int, int]:
    ...
    for _, row in df.iterrows():
        ...
        try:
            ...
            new += 1
            log.debug("  [NEW] [%-10s] %-55s | %-30s | %s",
                      site_label, (title or "?")[:55], (location_str or "?")[:30], url)
        except sqlite3.IntegrityError:
            existing += 1
            log.debug("  [DUP] [%-10s] %-55s | %s",
                      site_label, (title or "?")[:55], url)

    conn.commit()
    log.info("  Stored: %d new  |  %d duplicates", new, existing)
    return new, existing
...
def _run_one_search(
    ...
    log.info(
        "► SEARCH  query=%-30s  location=%-20s  boards=%-20s  hours_old=%d%s",
        f"'{s['query']}'",
        f"'{s['location']}'",
        str(other_sites + (["glassdoor"] if has_glassdoor else [])),
        hours_old,
    ...
    if other_sites:
        ...
        try:
            log.info("  Scraping %s...", other_sites)
            df = _scrape_with_retry(kwargs, max_retries=max_retries)
            all_dfs.append(df)
            if "site" in df.columns:
                for board, count in df["site"].value_counts().items():
                    log.info("  %-15s → %d raw results", board, count)
            else:
                log.info("  → %d raw results", len(df))
        except Exception as e:
    ...
    msg = f"[{label}] {before} scraped → {new} new, {existing} dupes"
    if filtered:
